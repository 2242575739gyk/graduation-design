...
//                        .singleResult();
    ...
    public Page<BizLeaveVo> findTodoTasks(BizLeaveVo leave, String userId) {
        ...
        List<Task> tasks = processService.findTodoTasks(userId, leave.getType(), leave.getInstanceId());
        // 根据流程的业务ID查询实体并关联
        for (Task task : tasks) {
    ...
    public Page<BizLeaveVo> findDoneTasks(BizLeaveVo bizLeave, String userId) {
        ...
        List<BizLeaveVo> results = new ArrayList<>();
        List<HistoricTaskInstance> hisList = processService.findDoneTasks(userId, bizLeave.getType(), bizLeave.getInstanceId());
        // 根据流程的业务ID查询实体并关联
